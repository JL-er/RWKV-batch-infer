...
class RWKV(MyModule):
    ...
    @MyFunction
    def ffn_seq(self, x, sx, ln_w, ln_b, k_mix, r_mix, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry,mask):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        xx = xx.mul_(mask.unsqueeze(-1))
    ...
    @MyFunction
    def ffn_one_v6(self, x, sx, ln_w, ln_b, k_maa, r_maa, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry,mask):
    ...
    @MyFunction
    def ffn_seq_v6(self, x, sx, ln_w, ln_b, k_maa, r_maa, kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry,mask):
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        #if mask!=None:
        xx = xx.mul_(mask.unsqueeze(-1))
    ...
    @MyFunction
    def att_one_v6_0(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory,mask):
        B=x.shape[0]
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        
        sx = sx - xx
        xxx = xx + sx * x_maa
        xxx = torch.tanh(xxx @ tm_w1).view(B, 5, 1, -1)
        xxx = (xxx@tm_w2).view(B, 5, -1)
        ...
        r = matmul(rx, rw, rmx, rrx, rmy, rry, output_dtype=torch.float32).view(B, H, 1, N)
        k = matmul(kx, kw, kmx, krx, kmy, kry, output_dtype=torch.float32).view(B, H, N, 1)
        v = matmul(vx, vw, vmx, vrx, vmy, vry, output_dtype=torch.float32).view(B, H, 1, N)
        g = F.silu(matmul(gx, gw, gmx, grx, gmy, gry))

        
        w = t_decay + (torch.tanh(wx @ td_w1) @ td_w2).float().view(B, H, N, 1)
    ...
    @MyFunction
    def att_seq_v6_0(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory,mask):
        H = t_decay.shape[0]
        N = x.shape[-1] // H
        T = x.shape[1]
        B = x.shape[0]
        ...
        xx = F.layer_norm(x, (x.shape[-1],), weight=ln_w, bias=ln_b)
        xx = xx.mul_(mask.unsqueeze(-1))
        sx = torch.cat((sx.unsqueeze(1), xx[:,:-1,:]),1) - xx
        xxx = xx + sx * x_maa

        xxx = torch.tanh(xxx @ tm_w1).view(B,T, 5, -1).transpose(1, 2)
        xxx = (xxx@tm_w2).view(B, 5, T, -1)
        mw, mk, mv, mr, mg = xxx.unbind(dim=1)
        ...
        r = matmul(rx, rw, rmx, rrx, rmy, rry, output_dtype=torch.float32).view(B,T, H, N).transpose(1, 2)
        k = matmul(kx, kw, kmx, krx, kmy, kry, output_dtype=torch.float32).view(B,T, H, N).permute(0, 2, 3, 1)
        v = matmul(vx, vw, vmx, vrx, vmy, vry, output_dtype=torch.float32).view(B,T, H, N).transpose(1, 2)
    
        g = F.silu(matmul(gx, gw, gmx, grx, gmy, gry))

        w = t_decay.view(1,1, H, N, 1) + (torch.tanh(wx @ td_w1) @ td_w2).float().view(B,T, H, N, 1)
        w = torch.exp(-torch.exp(w.float()))
        out = torch.empty((B,T, H, N), dtype=r.dtype, device=r.device)
        ...
        for t in range(T):
            ...
            vt = v[:,:,t:t+1,:]
            at = kt@vt
            out[:,t] = (rt @ (t_first * at + s)).squeeze(2)
            s = at + w[:,t] * s
        out = out.reshape(B*T, H*N)

        out = F.group_norm(out, num_groups=H, weight=lx_w, bias=lx_b, eps = 64e-5).view(B,T,-1)
        out = out.to(dtype=x.dtype) * g
        out = matmul(out, ow, omx, orx, omy, ory)
    ...
    ########################################################################################################
    if os.environ["RWKV_fla_ON"] == '1':
        @MyFunction
        def v6_0_before(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory, mask):
            H = t_decay.shape[0]
            N = x.shape[-1] // H
            T = x.shape[1]
            B = x.shape[0]
            ...
            r = matmul(rx, rw, rmx, rrx, rmy, rry, output_dtype=torch.float32).view(B,T, H, N).transpose(1, 2)
            k = matmul(kx, kw, kmx, krx, kmy, kry, output_dtype=torch.float32).view(B,T, H, N).transpose(1, 2)
            v = matmul(vx, vw, vmx, vrx, vmy, vry, output_dtype=torch.float32)

            g = F.silu(matmul(gx, gw, gmx, grx, gmy, gry))

            w = w = (t_decay.view(1,1, H, N) + (torch.tanh(wx @ td_w1) @ td_w2).float().view(B,T, H, N)).transpose(1, 2)
            w = -torch.exp(w.float())
            return r, k, v, g, w, xx[:,-1,:], s
        def fla_att_seq_v6_0(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory,mask):
            H = t_decay.shape[0]
            N = x.shape[-1] // H
            T = x.shape[1]
            B = x.shape[0]
            r, k, v, g, w, xxx, ss = self.v6_0_before(x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory, mask)
            if mask is not None:
                v = v.mul_(mask.unsqueeze(-1))
            v = v.view(B,T, H, N).transpose(1, 2)
            out, s = chunk_rwkv6(r, k, v, w, t_first,
                                             scale=1.,
                                             initial_state=ss,
    ...
    if os.environ["RWKV_CUDA_ON"] == '1':
        ...
        def v6_0_before(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory,mask):
            ...
            xxx = xx + sx * x_maa
            xxx = torch.tanh(xxx @ tm_w1).view(B,T, 5, -1).transpose(1, 2)
            xxx = (xxx@tm_w2.expand(B,tm_w2.shape[0],tm_w2.shape[1],tm_w2.shape[2])).view(B, 5, T, -1)
            ...
            w = t_decay.view(1,1, H, N) + (torch.tanh(wx @ td_w1) @ td_w2).float().view(B, T, H, N)

            return r, k, v, g, w, xx[:, -1,:], s.transpose(-1,-2).contiguous()

        def cuda_att_seq_v6_0(self, x, sx, s, ln_w, ln_b, lx_w, lx_b, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, t_decay, t_first, kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory, mask):
            H = t_decay.shape[0]
            N = x.shape[-1] // H
            T = x.shape[1]
            B = x.shape[0]
            ...
            out, s = self.RUN_RWKV_6(B, T, self.args.n_att, H, ss, r, k, v, w=w, u=t_first)
   
            return self.v5_2_after(t_decay, out, s, x, xxx, g, lx_w, lx_b, ow, omx, orx, omy, ory)

    ########################################################################################################

    def forward(self, tokens, state, mask=None, full_output=False):
        with torch.no_grad():
            w = self.w
            args = self.args
            B = tokens.shape[0]
            ...
            if state == None:
                if self.version == 4:
                    ...
                    for i in range(args.n_layer): # state: 0=att_xx 1=att_kv 2=ffn_xx
                        ...
                        dev = dd.device
                        atype = dd.atype
                        state[i*3+0] = torch.zeros(B, args.n_embd, dtype=atype, requires_grad=False, device=dev).contiguous()
                        if args.time_state:
                            state[i*3+1] = w[f'blocks.{i}.att.time_state'].transpose(1,2).to(dtype=torch.float, device=dev).requires_grad_(False).contiguous()
                        else:
                            state[i*3+1] = torch.zeros((B, args.n_head, args.n_att//args.n_head, args.n_att//args.n_head), dtype=torch.float, requires_grad=False, device=dev).contiguous()
                        state[i*3+2] = torch.zeros(B, args.n_embd, dtype=atype, requires_grad=False, device=dev).contiguous()
